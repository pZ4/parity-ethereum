...
use builtin::Builtin;
use encoded;
use engines::{EthEngine, NullEngine, InstantSeal, BasicAuthority, AuthorityRound, Tendermint, TLEngine, DEFAULT_BLOCKHASH_CONTRACT};
use error::Error;
	...
	fn engine(
		...
		match engine_spec {
			...
			ethjson::spec::Engine::TLEngine(tl_engine) => TLEngine::new(tl_engine.params.into(), machine)
				.expect("Failed to start TLEngine consensus engine."),
		}
	}
